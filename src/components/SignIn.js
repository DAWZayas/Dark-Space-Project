    ...
    return (
      <Modal isOpen={isOpen} onRequestHide={() => this.handleCancelClick()} backdrop keyboard>
        ...
        <div className="modal-body">
          <button className="Margin btn" type="button" onClick={ () => this.handlesignInWithGithubClick()}>GitHub</button>
          <button className="Margin btn" type="button" onClick={ () => this.handlesignInWithTwitterClick()}>Twitter</button>
          <button className="Margin btn" type="button" onClick={ () => this.handlesignInWithGoogleClick()}>Google</button>
        </div>
      </Modal>
    );
  }
