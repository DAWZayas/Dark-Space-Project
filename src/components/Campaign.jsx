...
export default class Campaign extends Component {
  ...
  render() {
    ...
    let lastMissionCompleted = 0;
    let user = '';
    campaigns.map( (campaign, index) => (points.missionpoints[index] * 100 / 500 >= 50) ? lastMissionCompleted++ : null);
        ...
        { auth.authenticated ?
          <div>
            <div className="col-xs-12 paddingLeft">
              ...
              <ul>
                {
                   campaigns.map( (campaign, index) => <CampaignItem key={index} id={index} campaign={campaign} points={points.missionpoints[index]} onRemoveCampaign={onRemoveCampaign} onRemoveMissionForPoints={onRemoveMissionForPoints} lastMissionCompleted={lastMissionCompleted - 1} admin={ (this.admin(firebase, auth.id)) }/>)
                }
              </ul>
            }
            </div>
